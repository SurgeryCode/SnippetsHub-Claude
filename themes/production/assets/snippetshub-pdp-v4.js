/**
 * SnippetsHub PDP v4 - Strict 1:1 Literal JS
 * Extracted directly from pdp-snippetshub-updated _v1.html
 */

document.addEventListener('DOMContentLoaded', function() {
    const wrapper = document.querySelector('.snippetshub-pdp-v4-wrapper');
    if (!wrapper) return;
    ...
    // Add to cart functionality (Console logs only for literal port)
    const addToCartBtn = wrapper.querySelector('#addToCart');

    addToCartBtn?.addEventListener('click', function() {
        if (!quantityInput) return;
        const quantity = quantityInput.value;
        const selectedVariant = wrapper.querySelector('.variant-option.active')?.textContent;
        const price = wrapper.querySelector('.price-current')?.textContent;

        console.log('Adding to cart (Literal Preview):', {
            product: 'Snippet Collection',
            variant: selectedVariant,
            quantity: quantity,
            price: price
        });

        this.innerHTML = '<span>Added to Cart!</span><span>✓</span>';
        this.style.background = 'var(--secondary)';
        
        setTimeout(() => {
            this.innerHTML = '<span>Add to Cart</span><span>→</span>';
            this.style.background = '';
        }, 2000);
    });

    // Scroll to top
    const scrollToTopBtn = wrapper.querySelector('#scrollToTop');
